...
Class Scintilla extends Scintilla.ScintillaBase {
    ...
    __New(gui, opts := "", DllPath := "Scintilla.dll", Style := 0x50010000, ExStyle := 0x200) {
        this.gui := gui  ; store reference to the parent gui object
        
        if (DllPath) {
            Scintilla.dllPath := dllPath
        }
        
        If (!Scintilla.controlCount) { ; Load the SciLexer dll if this is the first instance of Scintilla
            If !(Scintilla.dllHwnd := DllCall("LoadLibrary", "Str", Scintilla.dllPath)) {
        ...
        this.ctrl := this.gui.addCustom("ClassScintilla " opts " " Style . (ExStyle ? " +E" ExStyle : "")) ; Store reference to the GuiControl object
        this.ctlHwnd := this.ctrl.hwnd
        
        this.df := SendMessage(Scintilla.SCI_GETDIRECTFUNCTION, 0, 0, this.ctlHwnd) ; Used for sending messages directly to the control
        this.dp := SendMessage(Scintilla.SCI_GETDIRECTPOINTER, 0, 0, this.ctlHwnd)
        
        Scintilla.controlCount++ ; increment control count, which is seen by all instances of Scintilla
    }
    ...
    */
    ...
    __sciNotify(cb, notification, ctrl, lParam) {
        ...
        this.SCNCode         := NumGet(lParam + A_Ptrsize * 2,"UInt")

        this.Position        := NumGet(lParam + OffPosition,"Int")
        this.Ch              := NumGet(lParam + OffCh,"Int")
        this.Modifiers       := NumGet(lParam + OffModifiers,"Int")
        this.ModType         := NumGet(lParam + OffModType,"Int")
        this.Text            := NumGet(lParam + OffText,"Ptr")
        this.Length          := NumGet(lParam + OffLength,"Int")
        this.LinesAdded      := NumGet(lParam + OffLinesAdded,"Int")
